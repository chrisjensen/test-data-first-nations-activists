...
describe('Geographic Diversity', () => {
  ...
  it('should represent diverse Indigenous groups and cultures', () => {
    ...
    const culturalTagCounts: Record<string, number> = {};
  ...
  it('should include Pacific Islander representation', () => {
    ...
    const pacificRepresentation = people.filter(person => 
      pacificCountries.includes(person.address?.country || '') ||
  ...
  it('should include European Indigenous representation', () => {
    const people = firstNationsActivistsData.people;
    const europeanIndigenous = people.filter(person => 
      person.tags.some(tag => 
        tag.includes('sami') || 
        tag.includes('romani') || 
        tag.includes('basque')
  ...
  it('should not be dominated by historical figures', () => {
    ...
    const historicalCount = people.filter(person => 
      historicalNames.includes(person.fullName)
    ).length;
