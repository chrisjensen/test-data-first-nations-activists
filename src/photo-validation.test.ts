...
async function validateImageUrl(url: string): Promise<{ valid: boolean; status?: number; contentType?: string; error?: string }> {
  try {
...
describe('Photo URL Validation', () => {
  it('should have valid image URLs for all people with pictures', async () => {
    ...
    const unexpectedInvalidUrls = invalidUrls.filter(({ url }) => 
      !expectedInvalidUrls.some(placeholder => url.includes(placeholder))
    );
  ...
  it('should not use placeholder URLs in production', () => {
    ...
    const placeholderUrls = people.filter(person => 
      person.picture && (
        person.picture.includes('placeholder.test') ||
  ...
  it('should have consistent URL patterns for Wikipedia images', () => {
    const people = firstNationsActivistsData.people;
    const wikipediaImageUrls = people.filter(person => 
      person.picture && person.picture.includes('wikipedia.org')
    );
